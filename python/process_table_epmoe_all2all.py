...
def process_data(dense_gemm_file: str, group_gemm_file: str, batch_gemm_file: str, mla_file: str, output_path: str, output_prefix: str):
    ...
    for d, tp, num_groups, b_mla, m_per_group in configs:
        ...
        base_result = {
            ...
            'Up_Gemm(us)': up_gemm,
            'Down_Gemm(us)': 0,
            'AlltoAll_0(us)': dispatch_alltoall,
            'AlltoAll_1(us)': combine_alltoall,
    ...
    no_microbatch_outfile = os.path.join(
        output_path, output_prefix + 'epmoe-all2all-no-microbatch-overlapping.csv')
    two_microbatch_outfile = os.path.join(
        output_path, output_prefix + 'epmoe-all2all-two-microbatch-overlapping.csv')
    single_batch_outfile = os.path.join(
        output_path, output_prefix + 'epmoe-all2all-single-batch-comp-comm-overlapping.csv')
